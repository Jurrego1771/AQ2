...
test.describe('Live Stream detail — secciones inexploradas @regression @live-stream', () => {
  ...
  test('Start Recording arranca disabled: falta encoding profile y status published @LIVE-TC-117', async ({
    page,
    liveStreamDetailPage,
    liveStream,
  }) => {
    await liveStreamDetailPage.gotoDetail(liveStream);
    await expect
      .poll(() => liveStreamDetailPage.recordingButton.isDisabled(), { timeout: 15_000 })
      .toBe(true);
    // tiempo-recording enumera los motivos (visible aunque el boton este disabled).
    await expect
      .poll(async () => (await liveStreamDetailPage.timeRecording.count()) > 0, { timeout: 15_000 })
      .toBe(true);
    const errorsText = (await liveStreamDetailPage.timeRecording.first().innerText()).toLowerCase();
    expect(errorsText).toMatch(/encoding profile|published/);
    void page;
  });
