...
test.describe('Live Stream schedules — contrato API @api @live-stream', () => {
  ...
  // del bug #23 en description). Verificado en vivo (dev v7.0.70).
  ...
  // --- Prueba viva del bug #23: el vaciado de TEXTO no persiste en update ---
  // AJUSTE 2026-07-09: is_featured NO es parte del bug. Es un valor unidireccional
  // fastchannel -> schedule (el usuario NO edita is_featured desde el form de
  // schedule; viene seteado por el motor de fastchannel). Por eso el server,
  // correctamente, ignora un is_featured:false en el update del schedule (no es
  // un bug, es el diseno). El bug #23 aplica solo a campos que el USUARIO puede
  // editar desde el form (description, y por extension not_sellable/is_blackout
  // si su edicion es user-driven). El server trata TODOS con el mismo guard
  // "if(body.X)" (schedule-job/utils/index.js:473-484), pero el campo que cae
  // correctamente 'ignorado' por ese diseno no es bug — es API contract.
  test('vaciar la descripción en update debe persistir [BUG #23] @LIVE-TC-13', async ({
    liveStreamClient,
    liveStream,
  }) => {
    test.fail(
      true,
      'BUG #23: el update ignora description="" — https://github.com/Jurrego1771/AQ2/issues/23'
    );
    const day = dayStr(50);
    ...
    expect(updRes.status(), 'el update responde OK (falsa sensacion de guardado)').toBe(200);

    const after = (await (await liveStreamClient.scheduleJob(liveStream, sid)).json()).data;
    expect(after.description, 'la descripcion vaciada debe persistir').toBe('');
  });
});
